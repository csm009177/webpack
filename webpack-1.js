 ...
 * ctrl+F 를 통해 아래의 정보를 따로 확인하시기 바랍니다
 * 1. isProduction
 * - 여기서 설명하는 process.env.NODE_ENV 는 Node.js 에서 제공하는 환경변수이다
 * - == 동등연산자를 사용하는 것이 마음에 들지 않지만, 검증된 설정 파일 자동으로 세팅한 것이므로 확인만 한다 
 * 2. config 객체
 * 3. module.exports
 * 
...
const HtmlWebpackPlugin = require("html-webpack-HtmlWebpackPlugin");
 ...
 * * 난이도 높으 설명: exclude는 로더가 처리하지 않을 파일에 대한 결로를 지정하는 문자열이다
 * 
 * * 기타 속성으 ㄴ로더가 처리할 파일에 대한 경로를 지정하는 문자열이다
 * * 로더는 오른쪽에서 왼쪽으로 실행된다 ★(매우 중요함)
 * * 배열의 마지막 순서부터 읽어 들인다
 */
module:{
  rules:[
